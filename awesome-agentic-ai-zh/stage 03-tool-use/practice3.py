...
tool message 沒帶 tool_call_id——LLM 無法配對哪個 result 對應哪個 call
沒 max_iter——tool 結果寫不好時、LLM 會無限呼叫，safety net 必須設
"""
